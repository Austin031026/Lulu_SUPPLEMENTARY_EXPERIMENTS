...
def _records(snapshot, method, *, include_empty=False):
    ...
    with torch.no_grad():
        ...
        for record, causal, hindsight in zip(records, c, h):
            ...
            if method == 'ren_resolved':
                from lulu.resolved import mismatch_scores
                record['resolved_weight'] = mismatch_scores(frozen_head(causal), frozen_head(hindsight),
                    teacher_head(record['teacher_hidden']), 3)['raw_weight']
            elif method == 'ren_opd':
...
def _dense_loss(student, records, frozen_head, teacher_head, tok, method):
    ...
    for r, live in zip(records, hidden):
        ...
        if method == 'ren_resolved':
            total = total + (r['resolved_weight'] * forward_kl(head(live), teacher_logits.softmax(-1), reduction='none')).sum()
        elif method == 'ren_opd':
...
def test_two_rank_update_matches_global_nonempty_sequence_mean_with_dummy_rank(tmp_path):
    ...
    training.atomic_torch(cache / 'teacher_head.pt', {'weight': teacher_head.weight, 'bias': None})
