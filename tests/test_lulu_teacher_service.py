...
@pytest.mark.parametrize('tp_mode',['native','eager-local'])
def test_native_tp_two_gpu_matches_dense_model_across_rounds(tmp_path,tp_mode):
    ...
    args = _args('ren_graft')
    args.teacher_tp_mode=tp_mode
    ...
    try:
        ...
        assert ready['control_backend'] == 'gloo'
        assert ready['nccl_initialization'] == 'synchronous_warmup'
...
def test_teacher_groups_warmup_nccl_before_cpu_control(monkeypatch):
    calls=[]
    monkeypatch.setattr(teacher_service.dist,'init_process_group',lambda *a,**kw:calls.append(('init',a,kw)))
    monkeypatch.setattr(teacher_service.dist,'barrier',lambda **kw:calls.append(('barrier',kw)))
    monkeypatch.setattr(teacher_service.torch.cuda,'synchronize',lambda device:calls.append(('sync',device)))
    def new_group(**kw):
        calls.append(('control',kw));return 'cpu-control'
    monkeypatch.setattr(teacher_service.dist,'new_group',new_group)
    group=teacher_service.initialize_teacher_groups(1,2,torch.device('cuda:1'),'127.0.0.1',12345,90)
    assert group=='cpu-control'
    assert [c[0] for c in calls]==['init','barrier','sync','control']
    assert calls[0][1]==('nccl',) and 'device_id' not in calls[0][2]
    assert calls[1][1]['device_ids']==[1]
    assert calls[3][1]['backend']=='gloo'


def test_teacher_uses_cpu_control_for_requests_and_stop(monkeypatch):
    _mock_loader(monkeypatch)
    args=_args();args.cpu=False
    conn=_Connection([_request(),{'op':'stop'}]);calls=[]
    monkeypatch.setattr(teacher_service.torch.cuda,'set_device',lambda d:None)
    monkeypatch.setenv('CUDA_VISIBLE_DEVICES','test')
    monkeypatch.setattr(teacher_service,'initialize_teacher_groups',lambda *a:'cpu-control')
    monkeypatch.setattr(teacher_service.dist,'is_initialized',lambda:True)
    monkeypatch.setattr(teacher_service.dist,'barrier',lambda **kw:calls.append(('barrier',kw)))
    monkeypatch.setattr(teacher_service.dist,'broadcast_object_list',lambda payload,**kw:calls.append(('broadcast',kw)))
    monkeypatch.setattr(teacher_service.dist,'destroy_process_group',lambda *a:calls.append(('destroy',a)))
    monkeypatch.setattr(teacher_service,'score_records',lambda *a:[])
    teacher_service.teacher_worker(args,0,2,['6','7'],'127.0.0.1',12345,conn)
    broadcasts=[kw for op,kw in calls if op=='broadcast']
    assert len(broadcasts)==2
    assert all(kw['group']=='cpu-control' and kw['device'].type=='cpu' for kw in broadcasts)
    assert [v for op,v in calls if op=='destroy']==[('cpu-control',),()]
    assert conn.replies[0]['control_backend']=='gloo'
    assert conn.replies[-1]['op']=='stopped'
