...
METHODS = ('ren_shared', 'ren_balanced', 'ren_stable', 'ren_resolved', 'ren_opd', 'ren_graft', 'vanilla_opd', 'opsd', 'causal_topk', 'union_topk')
...
def parser():
    ...
    p.add_argument('--retain-checkpoints', default='', help='Extra completed round/update indices to retain')
    p.add_argument('--master-weights-fp32', action='store_true', help='Keep full-training weights and Adam states in FP32; use BF16 autocast for compute')
    p.add_argument('--rollout-backend', choices=('hf', 'vllm'), default='hf')
    p.add_argument('--rollout-top-k', type=int, default=0, help='Sampling Top-K, independent of recognition diagnostics')
    p.add_argument('--rollout-vllm-memory', type=float, default=.35)
    p.add_argument('--rollout-vllm-max-seqs', type=int, default=16)
    p.add_argument('--rollout-vllm-sleep', action='store_true', help='Sleep the resident vLLM engine between collection and update to release GPU memory')
    p.add_argument('--optimizer-state-sharding', action='store_true', help='Shard AdamW moments across Student DDP ranks, keeping full-parameter gradients and checkpoints')
    p.add_argument('--resolved-weight-epsilon', type=float, default=1e-8)
    p.add_argument('--reference-kl-coef', type=float, default=.1)
    p.add_argument('--control-loss-coef', type=float, default=1., help='Multiply the existing token-average answer/control Teacher KL')
    p.add_argument('--match-causal-update', action='store_true', help='Match causal scoring to single-record update batches and full head chunks; verify pre-update reasoning loss')
    p.add_argument('--stable-ratio-epsilon', type=float, default=1e-6)
    p.add_argument('--gradient-norm-every', type=int, default=0, help='Exact full-batch component gradient norms every N updates, plus the first')
    p.add_argument('--gradient-cosines', action='store_true', help='Exact pairwise component gradient cosines; rank0 CPU gradient archive')
    p.add_argument('--reasoning-ablation', choices=('ren','uniform','shuffled','causal_matched','vanilla','none','opsd'), default='ren', help='Balanced backbone allocation: ReN; budget-matched uniform/shuffled/causal controls; vanilla; control/reference only; or direct hindsight OPSD')
    p.add_argument('--reasoning-diagnostic-split', type=int, default=0, help='Absolute response-position boundary for reasoning statistics and gradient contributions; 0 disables')
    p.add_argument('--validation-data', help='Frozen disjoint prepared dev JSONL; never used for training')
    p.add_argument('--validation-every', type=int, default=0, help='Resident greedy validation at round0 and every N completed rounds')
    p.add_argument('--validation-max-examples', type=int, default=256)
    p.add_argument('--stability-early-stop', action=argparse.BooleanOptionalAction, default=True)
    p.add_argument('--stability-min-rounds', type=int, default=5)
    ...
    p.add_argument('--teacher-tp-mode', choices=('native', 'eager-local'), default='native')
...
def validate_args(a):
    if getattr(a, 'optimizer_state_sharding', False) and (a.backend != 'persistent' or a.lora_rank != 0):
        raise ValueError('Optimizer state sharding requires persistent full-parameter training')
    if getattr(a, 'rollout_vllm_sleep', False) and a.rollout_backend != 'vllm':
        raise ValueError('vLLM sleep requires the vLLM rollout backend')
    if not math.isfinite(getattr(a,'control_loss_coef',1.)) or getattr(a,'control_loss_coef',1.) < 0:
        raise ValueError('control-loss-coef must be finite and nonnegative')
    if getattr(a,'control_loss_coef',1.) != 1. and a.method not in ('ren_balanced','ren_stable','ren_shared'):
        raise ValueError('control-loss-coef requires the stable control/reference backbone')
    if getattr(a,'match_causal_update',False) and (a.method != 'ren_balanced' or a.train_micro_batch_size != 1 or a.update_passes != 1):
    ...
    if a.method in ('ren_resolved', 'ren_stable', 'ren_shared', 'ren_balanced') and (a.backend != 'persistent' or a.phase != 'run'):
        raise ValueError('Resolved/stable ReN requires the persistent round coordinator')
    if a.method in ('ren_stable', 'ren_shared', 'ren_balanced'):
        if a.lora_rank != 0:
            raise ValueError('ren_stable currently requires full-parameter Student training')
        if not math.isfinite(a.reference_kl_coef) or a.reference_kl_coef <= 0:
            raise ValueError('reference-kl-coef must be finite and positive')
        if not math.isfinite(a.stable_ratio_epsilon) or a.stable_ratio_epsilon <= 0:
            raise ValueError('stable-ratio-epsilon must be finite and positive')
        if a.stability_min_rounds < 3:
            raise ValueError('stability-min-rounds must be at least 3')
    if a.reasoning_diagnostic_split < 0 or a.gradient_norm_every < 0 or a.validation_every < 0 or a.validation_max_examples < 1:
        raise ValueError('Invalid gradient diagnostic or validation schedule')
    if a.validation_every and (not a.validation_data or (a.rollout_backend != 'vllm' and not a.cpu)):
        raise ValueError('Resident validation requires dev data and vLLM')
    if a.validation_every and a.method not in ('ren_stable', 'ren_shared', 'ren_balanced'):
        raise ValueError('Resident dev selection is supported on the stable backbone')
    if a.rollout_backend == 'vllm' and (a.cpu or a.backend != 'persistent'):
        raise ValueError('vLLM rollout requires persistent CUDA training')
    if a.rollout_top_k < 0 or not 0 < a.rollout_vllm_memory < 1 or a.rollout_vllm_max_seqs < 1:
        raise ValueError('Invalid rollout sampling or vLLM settings')
    if not math.isfinite(a.resolved_weight_epsilon) or a.resolved_weight_epsilon <= 0:
        raise ValueError('resolved-weight-epsilon must be finite and positive')
    if any(int(x) < 0 for x in a.retain_checkpoints.split(',') if x.strip()):
        raise ValueError('retain-checkpoints must contain nonnegative indices')
...
def load_student(a, checkpoint_dir=None, trainable=False):
    ...
    weight_dtype = torch.float32 if trainable and not a.lora_rank and getattr(a, 'master_weights_fp32', False) else dtype_for(a)
    model = AutoModelForCausalLM.from_pretrained(source, torch_dtype=weight_dtype,
...
def autocast_context(a):
    return torch.autocast('cuda', dtype=torch.bfloat16,
        enabled=not getattr(a, 'cpu', True) and getattr(a, 'dtype', 'float32') == 'bfloat16')
...
def selected_hidden(model, tok, records, view, a):
    ...
    # Native BF16 TP already has its compute dtype. Autocast around DTensor
    # redistributions can desynchronize functional collectives on torch 2.7.
    context = autocast_context(a) if base_model(model).get_input_embeddings().weight.dtype == torch.float32 else contextlib.nullcontext()
    with context:
...
class DistillationStep(nn.Module):
    ...
    def forward(self, records):
        with autocast_context(self.args):
            return self._forward(records)

    def _forward(self, records):
        if self.args.method in ('ren_stable', 'ren_shared', 'ren_balanced'):
            from lulu.stable import stable_forward
            return stable_forward(self, records)
        a = self.args
        ...
        for r, hs in zip(records, hidden):
            ...
            for start in range(0, n, a.logit_chunk_size):
                ...
                frozen = None if a.method == 'ren_resolved' else r['student_hidden'][start:end].to(hs.device)
                ...
                weights = r.get('resolved_weight')
                weights = weights[start:end].to(hs.device) if weights is not None else None

                def chunk_loss(live, frozen, ids, probs, th, recognition_ids, weights):
                    live_logits = head(live)
                    if a.method == 'ren_resolved':
                        target = self.teacher_head(th).detach().float().softmax(-1)
                        return (weights.detach() * forward_kl(live_logits, target, reduction='none')).sum()
                ...
                value = checkpoint(chunk_loss, hs[start:end], frozen, ids, probs, th, recognition_ids, weights,
                                   use_reentrant=False) if a.gradient_checkpointing else chunk_loss(hs[start:end], frozen, ids, probs, th, recognition_ids, weights)
                total = total + (value if a.method == 'ren_resolved' else value / n)
        return total
...
def run(a):
    ...
    plan = {'model': a.model, 'teacher': a.teacher_model, 'method': a.method, 'rounds': a.rounds,
            ...
            'rollout_policy': {'temperature': a.temperature, 'top_p': a.top_p, 'top_k': a.rollout_top_k}}
