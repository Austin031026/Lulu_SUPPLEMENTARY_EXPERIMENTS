...
_TEACHER_METHODS = frozenset(('ren_shared', 'ren_balanced', 'ren_stable', 'ren_resolved', 'ren_opd', 'ren_graft', 'causal_topk', 'union_topk', 'vanilla_opd'))
...
def sanitize_score_request(request, method, *, vocab_size=None, max_sequence_tokens=None):
    ...
    for record in request['records']:
        if method in ('vanilla_opd', 'ren_opd', 'ren_resolved', 'ren_stable', 'ren_shared', 'ren_balanced'):
...
def score_records(model, tokenizer, records, args):
    ...
    for start in range(0, len(records), args.score_batch_size):
        ...
        for record, hidden in zip(batch, states):
            ...
            if args.method in ('vanilla_opd', 'ren_opd', 'ren_resolved', 'ren_stable', 'ren_shared', 'ren_balanced'):
...
def initialize_teacher_groups(rank, world, device, address, port, timeout):
    """Keep CPU control waits separate from GPU reductions; initialize synchronously.

    The bound-device init path selected nonblocking NCCL communicators in our
    Torch 2.7 runtime. Use the regular communicator and explicitly complete a
    device-bound warmup before loading the model. This changes transport only.
    """
    dist.init_process_group('nccl', rank=rank, world_size=world,
        init_method=f'tcp://{address}:{port}', timeout=timedelta(seconds=timeout))
    dist.barrier(device_ids=[device.index])
    torch.cuda.synchronize(device)
    return dist.new_group(ranks=list(range(world)), backend='gloo',
...
def teacher_worker(args, rank, world, device_ids, address, port, conn):
    ...
    CPU Gloo group's broadcast; model reductions stay on NCCL. The parent must poll process exit codes as
    ...
    try:
        ...
        if world > 1:
            control_group = initialize_teacher_groups(rank, world, device, address, port,
                                                       getattr(args, 'worker_timeout', 1800))
        ...
        ready = {'op': 'ready', 'role': 'teacher', 'world': world,
                 ...
                 'control_backend': 'gloo' if world > 1 else 'pipe',
                 'nccl_initialization': 'synchronous_warmup' if world > 1 else None,
                 'tp_reductions': getattr(model, '_lulu_tp_runtime', None) or ('synchronous_rowwise' if world > 1 else None),
        ...
        while True:
            ...
            if world > 1:
                dist.broadcast_object_list(payload, src=0, group=control_group, device=torch.device('cpu'))
            request = payload[0]
            ...
            started = time.monotonic()
            if rank == 0:
                print(f'Teacher score start: round={request["round"]} request={request["request_id"]} records={len(request["records"])}', flush=True)
            records = score_records(model, tokenizer, request['records'], args)
            if rank == 0:
                print(f'Teacher score complete: round={request["round"]} request={request["request_id"]} seconds={time.monotonic()-started:.2f}', flush=True)
            if rank == 0:
