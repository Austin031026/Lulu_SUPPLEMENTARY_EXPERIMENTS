...
def structured_response_regions(tokenizer, response_ids, *, prompt_ids=None):
    """Partition sampled IDs by actual think markers, never answer-like prose.

    Marker tokens, the final-answer suffix and sampled EOS belong to control.
    A truncated open think span stays reasoning: no synthetic stop is appended.
    The concatenation of reasoning/control covers every sampled response token.
    """
    ids = list(map(int, response_ids))
    opening = list(tokenizer.encode('<think>', add_special_tokens=False))
    closing = list(tokenizer.encode('</think>', add_special_tokens=False))
    if not opening or not closing or opening == closing:
        raise ValueError('Tokenizer must distinguish the thinking boundary markers')
    prompt = tokenizer.decode(list(prompt_ids or []), skip_special_tokens=False,
                              clean_up_tokenization_spaces=False)
    inside = re.search(r'<think>\s*$', prompt) is not None
    special = set(getattr(tokenizer, 'all_special_ids', []))
    reasoning = [False] * len(ids)
    index = 0
    while index < len(ids):
        if ids[index:index+len(opening)] == opening:
            inside = True
            index += len(opening)
        elif ids[index:index+len(closing)] == closing:
            inside = False
            index += len(closing)
        else:
            reasoning[index] = inside and ids[index] not in special
            index += 1
    return {'reasoning': reasoning, 'control': [not flag for flag in reasoning]}
