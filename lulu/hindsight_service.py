...
def score_hindsight(model, tok, records, a):
    ...
    if not records:
        return []
    if a.method not in ('ren_shared', 'ren_balanced', 'ren_stable', 'ren_resolved', 'ren_opd', 'ren_graft', 'union_topk', 'causal_topk', 'opsd', 'vanilla_opd'):
    ...
    for start in range(0, len(records), a.score_batch_size):
        ...
        for offset, (record, h) in enumerate(zip(batch, hidden)):
            if a.method in ('ren_resolved', 'ren_stable', 'ren_shared', 'ren_balanced'):
                results.append({'hindsight_hidden': h.detach().cpu().clone()})
                continue
            if a.method == 'opsd':
...
def hindsight_worker(a, device_ids, conn):
    ...
    try:
        ...
        model.requires_grad_(False).eval()
        reference = None
        ready = {'op': 'ready', 'role': 'hindsight', 'sync_names': expected_names}
        if a.method in ('ren_stable', 'ren_shared', 'ren_balanced'):
            reference_path = Path(a.output_dir)/'checkpoints/round_000000'
            reference = load_student(a, checkpoint_dir=reference_path, trainable=True)
            reference.gradient_checkpointing_disable()
            disable_dropout(reference)
            reference.requires_grad_(False).eval()
            from lulu.training import head_state
            ready['reference_head'] = head_state(reference)
            ready['reference_checkpoint'] = str(reference_path)
        ...
        while True:
            ...
            if operation == 'stop':
                ...
                return
            if operation in ('sync', 'sync_checkpoint'):
                ...
                if operation == 'sync_checkpoint':
                    from lulu.weight_sync import sync_hindsight_checkpoint
                    sync_hindsight_checkpoint(model, request['checkpoint'], round_index)
                else:
                ...
                if reference is not None:
                    with torch.inference_mode():
                        for start in range(0, len(request['records']), a.score_batch_size):
                            batch = request['records'][start:start+a.score_batch_size]
                            states = selected_hidden(reference, tok, batch, 'causal_prompt_ids', a)
                            for offset, hidden in enumerate(states):
                                result[start+offset]['reference_hidden'] = hidden.detach().cpu().clone()
