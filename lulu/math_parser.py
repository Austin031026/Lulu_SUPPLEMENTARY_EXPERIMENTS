...
unit_texts = ["east", "degree", "mph", "kmph", "ft", "m sqaure", " m east", 
            "sq m", "deg", "mile", "q .", "monkey", "prime", "ratio", 
            "profit of rs", "rd", "o", "gm", "p . m", "lb", "tile", "per", 
            "dm", "lt", "gain", "ab", "way", "west", "a .", "b .", "c .", 
            "d .", "e .", "f .", "g .", "h .", "t", "a", "h", "no change", 
            "men", "soldier", "pie", "bc", "excess", "st", "inches", "noon", 
            "percent", "by", "gal", "kmh", "c", "acre", "rise", "a . m", "th", 
            "π r 2", "sq", "mark", "l", "toy", "coin", "sq . m", "gallon", 
            "° f", "profit", "minw", "yr", "women", "feet", "am", "pm", "hr", 
            "cu cm", "square", "v â € ™", "are", "rupee", "rounds", "cubic", 
            "cc", "mtr", "s", "ohm", "number", "kmph", "day", "hour", "minute", 
            "min", "second", "man", "woman", "sec", "cube", "mt", "sq inch", 
            "mp", "∏ cm ³", "hectare", "more", "sec", "unit", "cu . m", "cm 2", 
            "rs .", "rs", "kg", "g", "month", "km", "m", "cm", "mm", "apple", 
            "liter", "loss", "yard", "pure", "year", "increase", "decrease", 
...
def extract_answer(pred_str, data_name="", use_last_number=True):
    ...
    if data_name in ["mmlu_stem", "sat_math", "aqua", "gaokao2023", "arc_challenge", "truthfulqa"]:
        # TODO check multiple choice
        return choice_answer_clean(pred_str)
    if data_name.lower() in ["olympiadbench"]:
        boxes = []
        idx = 0
        while True:
            idx = pred_str.find("oxed{", idx)
            if idx == -1: break
            ...
            content = ""
            start_content = idx + 4
            for i in range(start_content, len(pred_str)):
                char = pred_str[i]
                if char == "{": balance += 1
                elif char == "}":
                    if balance == 0:
                        boxes.append(pred_str[start_content+1:i])
                        break
                    balance -= 1
            idx += 1 # 继续找下一个
        ...
        # 2. 决策逻辑
        if len(boxes) > 0:
            # 如果有多个盒子，通常对于 OlympiadBench，我们需要保留所有，或者尝试合并
            # 为了兼容 math_equal，我们可以返回最后 capture 的所有内容，或者拼接
            # 对于简单单值题，boxes[-1] 是对的
            # 对于多值题 (roots are 1, 2)，模型可能输出 \boxed{1}, \boxed{2}
            
            # 策略：如果 data_name 是 olympiadbench，尝试返回列表或特殊格式
            # 现有的 math_equal 可能不支持列表输入，所以最稳妥的是：
            # 如果有多个 box，就把它们拼成字符串返回，让 math_equal 去处理
            if len(boxes) > 1:
                return ", ".join(boxes) 
            return boxes[-1]
    ...
    if (
        ...
        or "mmlu" in data_name or data_name in ["arc_challenge", "truthfulqa"]
    ):  
...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    if data_name in ["math", "minerva_math"]:
        gt_cot = example.get("solution", example.get("answer"))
        ...
        if isinstance(ans, int):
            ...
            # 某些版本 dataset 可能用 'answer_index'
            idx = example.get("answer_index")
            abcd = "ABCDEFGHIJ"
            gt_ans = abcd[idx]   
        gt_cot = None
        ...
        # answerKey is typically one of A/B/C/D/E
        gt_cot = None
        gt_ans = str(example["answerKey"]).strip().upper()
    elif data_name == "truthfulqa":
        # Prefer EleutherAI/truthful_qa_mc style:
        #   question: str
        #   choices: List[str]
        #   label: "A"/"B"/"C"/"D" or int
        gt_cot = None
        if "label" in example:
            label = example.get("label")
        else:
            label = example["mc1_targets"]["labels"]
        if isinstance(label, int):
            gt_ans = "ABCD"[label]
        elif isinstance(label, list):
            correct_idx = label.index(1)
            gt_ans = chr(65 + correct_idx)
        else:
            gt_ans = str(label).strip().upper()
            # robust fallback if dataset returns longer text
            if len(gt_ans) > 1 and gt_ans in ["0", "1", "2", "3"]:
                gt_ans = "ABCD"[int(gt_ans)]
    elif data_name in [
        "aime24",
        "aime25",
        "amc23",
        ...
        gt_cot, gt_ans = None, example.get("answer", example.get("Answer"))
    else:
...
def parse_question(example, data_name):
    ...
    if data_name == "asdiv":
        ...
        options = example["options"] # MMLU-Pro 通常是 options 字段
        # MMLU-Pro 最多有 10 个选项
        labels = "ABCDEFGHIJ"
        formatted_options = []
        for i, option in enumerate(options):
            if i < len(labels):
                formatted_options.append(f"({labels[i]}) {str(option).strip()}")
        ...
        # allenai/ai2_arc format:
        # example["question"] is the stem
        # example["choices"] is a dict with fields "label" and "text"
        q = example["question"].strip()
        choice_labels = example["choices"]["label"]
        choice_texts = example["choices"]["text"]
        formatted_options = []
        for lab, txt in zip(choice_labels, choice_texts):
            formatted_options.append(f"({str(lab).strip()}) {str(txt).strip()}")
        options_str = "\n".join(formatted_options)
        question = (
            f"{q}\n"
            f"Answer Choices:\n{options_str}\n"
            f"Please reason step by step and output the final answer in the format: The answer is (X)."
        )
    elif data_name == "truthfulqa":
        # EleutherAI/truthful_qa_mc format:
        # example["question"], example["choices"], example["label"]
        q = example["question"].strip()
        labels = "ABCD"
        ...
        for i, txt in enumerate(choices):
            if i < len(labels):
                formatted_options.append(f"({labels[i]}) {str(txt).strip()}")
        options_str = "\n".join(formatted_options)
        question = (
            f"{q}\nAnswer Choices:\n{options_str}\n"
            f"Please reason step by step and output the final answer in the format: The answer is (X)."
        ...
        for key in ["question", "problem", "Problem", "Question", "input"]:
            if key in example:
...
PROMPT_TEMPLATES = {
    ...
    "qwen25-mmlu-pro": (
        ...
        "At the end of your response, output the final answer in the format: 'The answer is (X)'.\n"
        "<|im_end|>\n"
        "<|im_start|>assistant\n",
        "{output}",
        "\n\n",
...
def math_equal(prediction, reference, include_percentage=True, is_close=True):
    """
    升级版 math_equal: 支持列表/集合/元组的无序匹配
    """
    # 1. 预处理
    prediction = str(prediction).strip()
    reference = str(reference).strip()
    ...
    # 2. 尝试直接比较 (针对单个值)
    if is_single_math_equal(prediction, reference): 
        return True

    # 3. 尝试作为列表比较 (Split & Set Match)
    pred_parts = split_smart(prediction)
    ref_parts = split_smart(reference)

    # 如果拆分后没有产生多个元素，说明不是列表，直接返回 False (因为第2步已经试过单值了)
    if len(pred_parts) <= 1 and len(ref_parts) <= 1:
        return False
    ...
    # 4. 集合匹配 (贪心匹配：看 pred 的每个元素是否都能在 ref 里找到一个 math_equal 的)
    # 注意：为了处理重复元素（如根是 2, 2），需要用 visited 数组
    matched_indices = set()
    
    for p_item in pred_parts:
        found_match = False
        for i, r_item in enumerate(ref_parts):
            if i in matched_indices:
                continue
            
            # 递归调用 math_equal 来比较子元素
            # 这样也能处理嵌套结构，比如 list of tuples
            if math_equal(p_item, r_item): 
